...
"""Определения структур для взаимодействия с приборами Fastwel DIM760."""

from ctypes import Structure, c_uint8, c_uint16
from enum import IntEnum


class DIM760_DEBOUNCE(IntEnum):
    """Определение параметра "Устранение дребезга контактов" в конфигурации модуля DIM760."""
...
class DIM760_EDGE_SETTINGS(IntEnum):
    """Определение параметра "Конфигурация счетчика" в конфигурации модуля DIM760."""

    RISINGEDGE = 0          # Передний фронт
    FALLINGEDGE = 1         # Задний фронт


class DIM760_COUNTER_DIRECTION(IntEnum):
    """Определение параметра "Направление счета" в конфигурации модуля DIM760."""

    UPWARDCOUNT = 0         # Счет с суммированием
    DOWNWARDCOUNT = 1       # Счет с вычитанием


class DIM760_COUNTER_MODE(IntEnum):
...
class DIM760_COUNTER_INPUT_SETTINGS(Structure):
...
class DIM760_CONFIGURATION(Structure):
    ...
    _fields_ = [
        ...
        ("countingParameters", DIM760_COUNTER_INPUT_SETTINGS * 2),  # Конфигурация счетчика 1..2
    ]


class DIM760_INPUTS(Structure):
    """Область входных каналов "Inputs" модуля DIM760."""
...
class DIM760(Structure):
    """Структура, представляющая модуль DIM760."""

    _pack_ = 1
    _fields_ = [
        ("Inputs", DIM760_INPUTS),                  # Область входных параметров Inputs модуля
        ("Configuration", DIM760_CONFIGURATION),    # Область конфигурационных параметров модуля
    ]

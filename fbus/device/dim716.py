...
"""Определения структур для взаимодействия с приборами Fastwel DIM716."""

from ctypes import Structure, c_uint8, c_uint16
from enum import IntEnum


class DIM716_DEBOUNCE(IntEnum):
    """Определение параметра "Устранение дребезга контактов" в конфигурации модуля DIM716."""
...
class DIM716_EDGE_SETTINGS(IntEnum):
    """Определение параметра "Конфигурация счетчика" в конфигурации модуля DIM716."""

    RISINGEDGE = 0          # Передний фронт
    FALLINGEDGE = 1         # Задний фронт


class DIM716_COUNTER_DIRECTION(IntEnum):
    """Определение параметра "Направление счета" в конфигурации модуля DIM716."""

    UPWARDCOUNT = 0         # Счет с суммированием
    DOWNWARDCOUNT = 1       # Счет с вычитанием


class DIM716_COUNTER_MODE(IntEnum):
...
class DIM716_COUNTER_INPUT_SETTINGS(Structure):
...
class DIM716_CONFIGURATION(Structure):
    ...
    _fields_ = [
        ...
        ("countingParameters", DIM716_COUNTER_INPUT_SETTINGS * 2),  # Конфигурация счетчика 1..2
    ]


class DIM716_INPUTS(Structure):
    """Область входных каналов "Inputs" модуля DIM716."""
...
class DIM716(Structure):
    """Структура, представляющая модуль DIM716."""

    _pack_ = 1
    _fields_ = [
        ("Inputs", DIM716_INPUTS),                  # Область входных параметров Inputs модуля
        ("Configuration", DIM716_CONFIGURATION),    # Область конфигурационных параметров модуля
    ]

...
"""Определения структур для взаимодействия с приборами Fastwel NIM741, NIM742."""

from ctypes import Structure, c_uint8, c_uint16
from enum import IntEnum


class NIM74X_UART_BAUDRATE(IntEnum):
    """Скорость обмена."""
...
class NIM74X_UART_DATABITS(IntEnum):
...
class NIM74X_UART_STOPBITS(IntEnum):
...
class NIM74X_UART_PARITY(IntEnum):
    """Четность."""
...
class NIM74X_CONFIGURATION(Structure):
...
class NIM74X_INPUTS(Structure):
    """Области входных каналов "Inputs" модулей NIM741, NIM742."""
...
class NIM74X_OUTPUTS(Structure):
    """Области выходных каналов "Outputs" модулей NIM741, NIM742."""
...
class NIM74X(Structure):
    """Структура, представляющая модуль NIM741, NIM742."""

    _pack_ = 1
    _fields_ = [
        ("Inputs", NIM74X_INPUTS),                  # Область входных параметров Inputs модуля
        ("Outputs", NIM74X_OUTPUTS),                # Область выходных параметров Outputs модуля
        ("Configuration", NIM74X_CONFIGURATION),    # Область конфигурационных параметров модуля
    ]
